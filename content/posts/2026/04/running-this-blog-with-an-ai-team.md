...
excerpt: "This blog now runs with a small AI team handling research, writing, engineering, and ops—while Joseph keeps the voice, editorial direction, and final decisions."
...
At its core, the site is still a personal blog. It runs on a modern static-site stack, and the workflow is straightforward by design: ideas get developed, reviewed, and then published. Readers do not need to care about every moving part behind that setup, but it is worth knowing that the site now has a clearer operating model. Instead of one person juggling every draft, layout tweak, build check, and publishing decision alone, there is now a team helping with specific responsibilities while keeping the overall voice anchored in one place.
...
Pulsar works on the technical plumbing that turns content into a live site — the build pipeline, deployment reliability, and the path from a draft to something readers can actually see.
